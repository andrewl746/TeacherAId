...
async function classifySubmission({ subject, concept, prompt, codeSnippet, correctAnswer, expectedReasoning, studentAnswer, studentReasoning }) {
...
async function generateQuestion({ subject, concept, difficulty, questionType, feedbackHistory, weakestConcept }) {
  ...
  const questionTypeDescriptions = {
    short_answer:     'a direct question requiring a concise written answer',
    explain_concept:  'ask the student to explain a concept in their own words',
    apply_knowledge:  'give a scenario and ask the student to apply what they know',
    analyse_example:  'provide an example and ask the student to analyse it',
    identify_error:   'show something with a mistake and ask the student to find and explain it'
  };
  const qtDesc = questionTypeDescriptions[questionType] || questionType;

  const text = await callClaude(`You are an expert ${subject || 'General'} teacher. Generate a diagnostic question targeting: "${concept}".
Difficulty: ${difficulty || 'medium'}
Question type: ${questionType} — this means: ${qtDesc}${fbCtx}${weakCtx}

The question must require students to explain their reasoning, not just give an answer.
${subject && subject.toLowerCase() !== 'programming' ? 'Do NOT include code snippets — this is not a programming question.' : ''}

Respond ONLY with valid JSON (no markdown, no backticks):
{
  "prompt": "The question text",
  "supportingMaterial": "Any equation, passage, data, or example students need to reference (use \\n for line breaks), or null if not needed",
  "correctAnswer": "The concise correct answer",
  "expectedReasoning": "The full reasoning or explanation a student should provide to demonstrate understanding",
  "concept": "${concept}",
  "subject": "${subject || 'General'}"
}`);
  return parseClaudeJSON(text);
...
Generate a class insights report. Respond ONLY with valid JSON (no markdown):
{
  ...
  "teacherSummary": "2-3 sentences summarizing class performance and what to do next",
...
Generate a student insights report. Respond ONLY with valid JSON (no markdown):
{
  ...
  "teacherSummary": "2-3 sentences about this student's understanding and what help they need",
  "strengths": ["..."]
}`);
  return parseClaudeJSON(text);
}
